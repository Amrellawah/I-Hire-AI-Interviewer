...
function CandidateFeedbackDialog({candidate}) {
    ...
    const renderSummary = () => {
        if (!feedback?.summary) return <p>No summary available</p>
        
        // If summary is a string, display it directly
        if (typeof feedback.summary === 'string') {
            return <p>{feedback.summary}</p>
        }
        
        // If somehow it is an array, join it
        if (Array.isArray(feedback.summary)) {
            return <p>{feedback.summary.join(' ')}</p>
        }
                ...
                <DialogContent>
                    ...
                    <DialogDescription asChild>
                        <div className='mt-5'>
                            ...
                            <div className={`p-5 mt-7 flex items-center justify-between rounded-md ${feedback?.recommended == 'No'?'bg-red-100':'bg-green-100'}`}>
                                <div>
                                    <h2 className={` font-bold ${feedback?.recommended == 'No'?'text-red-700':'text-green-700'}`}>Recommendation Msg:</h2>
                                    <p className={`${feedback?.recommended == 'No'?'text-red-500':'text-green-500'}`}>{feedback?.recommended}</p>
                                </div>
                                <Button className={`${feedback?.recommended == 'No'?'bg-red-700':'bg-green-700'}`}>Send Msg</Button>
                            </div>
                        </div>
                    </DialogDescription>
