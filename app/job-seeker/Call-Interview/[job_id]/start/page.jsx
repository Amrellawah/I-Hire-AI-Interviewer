...
function StartInterview() {
            ...
            messages: [
                {
                    role: "system",
                    content:`
            You are an Al voice assistant conducting interviews.
    ...
    const GenerateFeedback = async() => {
        ...
        const {data, error} = await db
            ...
            .select();
        console.log(data);
        router.replace('/Call-Interview/'+job_id+"/completed");
        setLoading(false);
    }
        ...
        <div className='p-20 lg:px-48 xl:px-50 '>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-7 mt-5'>
                ...
                <div className='bg-white h-[400px] rounded-lg border flex flex-col gap-3 items-center justify-center'>
                    <div className='relative'>
                    {activeUser &&<span className="absolute inset-0 rounded-full bg-blue-500 opacity-75 animate-ping" />}
                    <h2 className='text-2xl text-white bg-primary h-[50px] w-[50px] p-3 rounded-full px-5'>{interviewInfo?.userName[0]}</h2>
                    </div>
